...
import { dnd_adhoc_filters, dnd_adhoc_metric, dnd_adhoc_metrics, dnd_granularity_sqla, dnd_sort_by, dnd_secondary_metric, dnd_size, dnd_x, dnd_y, dndColumnsControl, dndEntity, dndGroupByControl, dndSeries, dnd_adhoc_metric_2 } from './dndControls';
...
const series_limit = {
  ...
  validators: [legacyValidateInteger],
  choices: formatSelectOptions(SERIES_LIMITS),
  description: t('Limits the number of series that get displayed. A sub query ' + '(or an extra phase where sub queries are not supported) is applied to limit ' + 'the number of series that get fetched and displayed. This feature is useful ' + 'when grouping by high cardinality dimension(s).')
};
const sort_by = {
  type: 'MetricsControl',
  label: t('Sort By'),
