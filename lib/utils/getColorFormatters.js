...
const getColorFunction = ({
  ...
  if (operator !== _types2.COMPARATOR.NONE && operator !== _types2.COMPARATOR.INVERSE && !_types2.MULTIPLE_VALUE_COMPARATORS.includes(operator) && targetValue === undefined) {
    return () => undefined;
  }
  ...
  switch (operator) {
    ...
    case _types2.COMPARATOR.INVERSE:
      minOpacity = MIN_OPACITY_UNBOUNDED;
  ...
  return value => {
    ...
    return rgbToRgba(colorScheme, getOpacity(value, cutoffValue, extremeValue, minOpacity, maxOpacity, operator === _types2.COMPARATOR.INVERSE));
  };
};
